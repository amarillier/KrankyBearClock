# Windows compile script for KrankyBearClock / KrankyBearClock-windows (Windows build + optional packaging)
...
Write-Host "KrankyBear Clock - Windows Compile Script" -ForegroundColor Cyan
...
Remove-Item -Path (Join-Path bin 'KrankyBearClock') -Force -ErrorAction SilentlyContinue
...
if ($All -or $Windows -or (-not $Linux -and -not $Current)) {
    ...
    go build -ldflags="$ldflags" -trimpath -o bin/KrankyBearClock-windows.exe
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($Package) {
            ...
            try {
                Copy-Item -Path (Join-Path $PSScriptRoot "bin/KrankyBearClock-windows.exe") -Destination (Join-Path $PSScriptRoot "KrankyBearClock-windows.exe") -Force
            } catch {
            ...
            if (Test-Path $InnoPath) {
                & "$InnoPath" "Inno/KrankyBearClock.iss"
                if ($LASTEXITCODE -eq 0) {
...
if ($All -or $Linux) {
    if ($env:OS -eq 'Windows_NT') {
        ...
        $env:GOOS = "linux"
        $env:GOARCH = "amd64"
        go build -ldflags="-s -w" -trimpath -o bin/KrankyBearClock-linux
        if ($LASTEXITCODE -eq 0) {
...
if ($Current) {
    ...
    go build -ldflags="-s -w" -trimpath -o bin/KrankyBearClock-windows
    if ($LASTEXITCODE -eq 0) {
...
if ($buildFailed) {
    ...
    Get-ChildItem -Path bin -Filter "*KrankyBearClock*" | Format-Table Name, Length -AutoSize
    ...
    if ($All -or $Windows -or (-not $Linux -and -not $Current)) {
        ...
        Write-Host "  go-winres extract bin\KrankyBearClock-windows.exe --dir extracted_resources" -ForegroundColor Cyan
    }
}
