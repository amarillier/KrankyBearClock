...
var alarmEnabledChecks [5]*widget.Check // Store references to enabled checkboxes for UI updates
...
func checkAlarms(a fyne.App) {
	...
	for i := 0; i < 5; i++ {
		...
		if alarm.Repeat == "once" {
			...
			alarm.Enabled = false
			saveAlarm(a, i)
			// Update UI if alarm window is open
...
func makeAlarmsWindow(a fyne.App, w fyne.Window, bg fyne.Canvas) {
	if alarmWindow != nil && alarmWindow.Content().Visible() {
		alarmWindow.Show()
	...
	for i := 0; i < 5; i++ {
		...
		alarmEnabledChecks[i] = enabledCheck // Store reference for dynamic updates
		...
		if alarm.SoundFile != "" {
			...
			parts := strings.Split(alarm.SoundFile, "/")
			filename := parts[len(parts)-1]
	...
	alarmWindow.SetCloseIntercept(func() {
		...
		for i := range alarmEnabledChecks {
			alarmEnabledChecks[i] = nil
		}
	})
...
// refreshAlarmWindowUI updates the UI for a specific alarm when it changes
