...
echo "KrankyBear Clock - Windows Compile (bash)"
echo "================================================"
echo
...
rm -f bin/KrankyBearClock-windows.exe || true
...
if ! go build -ldflags="$LDFLAGS" -trimpath -o bin/KrankyBearClock-windows.exe
then
...
then
  ...
  cp -f bin/KrankyBearClock-windows.exe ./KrankyBearClock.exe
  ISCC_DEFAULT="/c/Program Files (x86)/Inno Setup 6/ISCC.exe"
  if [[ -x "$ISCC_DEFAULT" || -f "$ISCC_DEFAULT" ]]
  then
    "${ISCC_DEFAULT}" ./Inno/KrankyBearClock.iss || {
